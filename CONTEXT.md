...
**Vault Browser**:
The navigation surface for browsing folders, nested folders, notes, and media assets in a vault as one mixed tree with terse explorer-style rows. Its compact creation, search, filter, and grouped upload/import controls stay reachable while the tree or result rows scroll beneath them. The vault root appears as `Vault` in the vault browser, while compact location breadcrumbs belong where context matters. Selecting a folder sets the active creation location. Within a location, folders sort before note and media rows, which then sort case-insensitively using natural alphanumeric sorting. An opened note or media asset reads as the strong selection state; the active creation folder uses a quieter marker. Detailed inputs show only when needed. Row actions can live in a compact overflow menu, with right-click context menus serving as a primary desktop interaction path. On narrow layouts, the browser tree is hidden in a sliding drawer toggle so the active workspace remains primary. The vault browser is UI language, not a separate domain entity.
