...
import { ImagePlus, MoreHorizontal, Move, Pencil, Trash2 } from "lucide-react";

import { renameMediaAssetAction, moveMediaAssetAction, trashMediaAssetAction } from "@/app/media/actions";
...
export function MediaRow({
  ...
  function handleDragStart(event: React.DragEvent) {
    event.dataTransfer.setData("application/filebucket", JSON.stringify({ type: "media", id: mediaAsset.id }));
      ...
      {menu?.mode === "rename" ? (
        <form
          onSubmit={async (event) => {
            ...
            window.dispatchEvent(new CustomEvent("vault-mutate", {
              detail: { type: "rename-media", mediaAssetId: mediaAsset.id, name: newName }
            }));
            setMenu(null);
        ...
        >
          ...
          <Input
            ...
            name="name"
            defaultValue={mediaAsset.filename}
            required
          />
